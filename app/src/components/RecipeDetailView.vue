...
<template>
  <section class="mx-auto w-full max-w-[1100px] px-4 pb-28 pt-7 sm:px-7 lg:pb-16 lg:pt-10">
    ...
    <article v-else-if="recipe" class="mt-5 overflow-hidden border-2 border-charcoal bg-porcelain shadow-[8px_8px_0_#242421] sm:shadow-[12px_12px_0_#242421]">
      ...
      <div class="px-5 py-6 sm:px-8 sm:py-9">
        ...
        <h1 class="mt-5 break-words font-display text-[clamp(2.8rem,9vw,5.8rem)] font-bold leading-[0.95] tracking-[-0.055em] text-charcoal">{{ recipe.title }}</h1>
        <p class="mt-5 max-w-[780px] text-lg leading-relaxed text-charcoal/72">{{ recipe.summary }}</p>
        ...
        <div class="mt-8 grid gap-8 border-t-2 border-charcoal/15 pt-8 md:grid-cols-2 md:gap-12">
          <section>
            ...
            <ul class="mt-5 grid gap-3">
              <li v-for="ingredient in recipe.ingredients" :key="ingredient" class="flex gap-3 border-b border-charcoal/12 pb-3 leading-relaxed">
                <span class="mt-2 size-2 shrink-0 rounded-full bg-blush" aria-hidden="true" />{{ ingredient }}
              </li>
            </ul>
          </section>
          ...
          <section>
            ...
            <ol class="mt-5 grid gap-5">
              <li v-for="(step, index) in recipe.steps" :key="`${index}-${step}`" class="grid grid-cols-[2.3rem_1fr] gap-3 leading-relaxed">
